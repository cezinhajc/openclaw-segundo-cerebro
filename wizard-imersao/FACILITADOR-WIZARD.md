...
Quando Hugo disser **"próximo"** ou **"avança"**, o agente envia dois itens em sequência:
...
Você co-apresenta junto com o Hugo. O que você escreve aparece na tela — os participantes leem. Escreva como quem está apresentando, não como quem está dando instrução ao apresentador.

Regra principal de navegação: **um slide por vez**.

- Cada `📎 slides/XX-nome.html` é um passo separado
- Quando o Hugo disser "próximo", entregue a próxima mensagem + o próximo slide — só isso
...
🎬 *Abrindo o repo ao vivo: `github.com/idens-openclaw-client-base` → navega pelo `cerebro/` → abre `cerebro/agentes/COMO-CONECTAR.md`*
...
Hugo pede: *"Transforma isso em skill"*
...
Hugo testa: *"Me mostra os leads esfriando"*
...
| registro-duvida-pendente | Registra dúvida que o bot não sabe + escala pro Hugo | ✅ Ativo |
...
|--------|---------|-----------|
| resumo-diario-suporte | 22h | Gera relatório do dia e envia pro Hugo via Telegram |
| consolidacao-kb-diaria | 23h | Analisa perguntas frequentes no Supabase e alimenta a base de conhecimento |

O efeito composto: quanto mais alunos perguntam, mais dados no Supabase, mais inteligente o bot fica, menos escalações pro Hugo. Em menos de 30 dias, -75% de suporte manual.
...
🎬 *Hugo abre o WhatsApp — mostra o histórico do OpenClawzinho.*
...
🎬 *Hugo abre o Telegram — mostra o bot funcionando hoje.*
...
**Hugo, manda uma pergunta ao vivo pro bot.**
...
**Quem conduz:** Hugo Okamoto (Estratégia de IA), Flávia (Arquitetura de IA) e Matheus Cardozo (Operação & Implementação). Acesso direto a quem já fez.
...
| Internet caiu | Hotspot celular. Flávia avisa o chat com horário de retorno. |
