...
Chamado pela rotina `resumo-diario-suporte` (cron diário às 22h) ou sob demanda quando o Hugo pedir.
...
2. **Calcular métricas**:
   ...
   - Dúvidas escaladas pro Hugo (lista com status)
3. **Montar resumo** no formato:
   ...
   ⚠️ Pendentes pro Hugo:
   - [dúvida] — status
   ```
4. **Enviar** via Telegram pro Hugo
...
- Mensagem formatada no Telegram do Hugo
